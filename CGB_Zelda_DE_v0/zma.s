...
;-------------<	ZBS >-------------
		...
		extern	PEACHBKST,KAB1BKST,KABABKST,BANKST,BANKS2,BANKS3
		extern	RMBGST,MSVRSV,OPBKST,OPBKS2,BKTRBD,TOUBGCH
;
;--------------	Ending---Chr.Tr. --------------
		extern	ENDCHA,ENDCHB2,ENDCHC,ENDCHB,ENDCHD,ENDCHE,ENDCHF
		extern	ENDCHG,ENDCHA1,ENDCHYM,ENDCHA2
;
;-------------<	ZMSSUB >-------------
		EXTERN	MESEGE
;
...
BANK1		GROUP	1
;-------------<	ZS1 >-------------
		EXTERN	DMASET,MSOMCL,GAKKIST,LOVEBKST,OAMCLR,OPA
		extern	TLX
;
;-------------<	ZTI >-------------
...
;-------------<	ZVD >-------------
		...
		extern	MAMUUBG,KABEDT1,TOUBGDT
;
;--------------	Ending---BG data --------------
		extern	ENDYMBG,ENDBS,EWAKUBG,EWAKUBG2,ENDBGA,ENDBGA1,ENDBGA2
		extern	ENDBGA3,ENDBGA4,ENDBGA5,ENDBGB2,ENDBGC,ENDBGB
		extern	ENDBGD,ENDBGE,ENDBGF,ENDBGG
;
;==============	バンク１４ ==============================================
BANK14		GROUP	14H
;-------------<	ZEX >-------------
...
;
	...
	JR	NZ,HMIT108
;
;;;12/07	LD	A,(FRCNT)	;ヨーロッパ版で変更
;;;12/07	RRCA			;	スポットライトを
;;;12/07	AND	%10000000	;	やめた
	LD	A,$000			;
	JR	HMIT10A		;秘密実験！
;
HMIT108
...
;(CGB)	DW	BANKS2
;(CGB)	DW	BANKS3
;(CGB)	DW	DJBKST	; Danjyon BG bank set !
;(CGB)	DW	GRBKST	; Danjyon BG bank set !
;(CGB)	DW	V_RAMB
;(CGB)	DW	BANKTR
;(CGB);;	DW	TITLCHST
;(CGB)	DW	VRRET
;(CGB)	DW	GMAPBKST
;(CGB)	DW	VRRET ;ODMBKST
;(CGB)	DW	GOVBKST
;(CGB)	DW	GMAPCHST
;(CGB)	DW	V_RAMD
;(CGB)	DW	OPBKST	; Opening bank(Sea & Mt.) set !
;(CGB)	DW	OPBKS2	; Opening bank(ZELDA) set !
;(CGB)	DW	PEACHBKST	;ピーチ写真！
;(CGB)	DW	LOVEBKST	;海岸デモ！
;(CGB)	DW	KAB1BKST	;へきが！
;(CGB);--- Ending - -
;(CGB)	DW	ENDCHA	;エンディング A CHR.TR
;(CGB)	DW	ENDCHB2	;エンディング B2 CHR.TR
;(CGB)	DW	ENDCHC	;エンディング C  CHR.TR
;(CGB)	DW	ENDCHB	;エンディング B  CHR.TR
;(CGB)	DW	ENDCHD	;エンディング D  CHR.TR
;(CGB)	DW	ENDCHE	;エンディング E  CHR.TR
;(CGB)	DW	ENDCHF	;エンディング F  CHR.TR
;(CGB)	DW	ENDCHG	;エンディング G  CHR.TR
;(CGB);
;(CGB)	DW	ENDCHA1	;エンディング A(地上）  CHR.TR
;(CGB)	DW	ENDCHYM	;エンディング A(山遠景）  CHR.TR
;(CGB)	DW	ENDCHA2	;エンディング A2(LINK OBJ) CHR.TR
;(CGB);- - - -
;(CGB)	DW	KABABKST ;カバの絵！
;(CGB)	DW	TOUBGCH	;７塔デモ！
;
;
;;	DW	SPTEST
